...
- **2026-06 — Panel de capacidad en producción (auto-medición)**: en Super Admin → **Salud del sistema** (`/admin/salud`) se agregó una tarjeta "Capacidad en vivo" + prueba de carga interna.
  ...
  - **2026-09 — Cobros SaaS (Stripe, solo ruta):** nueva página Super Admin **Cobros** (`/admin/cobros`, menú lateral) con MRR/ARR, ingresos por plan y tabla de clínicas (backend `routes/billing.py` → `GET /admin/billing/overview`, datos reales). Clínica: pestaña **Configuración → Plan** ahora muestra tarjeta "Facturación y pagos" con cargo mensual real (`GET /billing/subscription`) y botón "Administrar método de pago". La integración Stripe queda como **STUB** (`POST /billing/checkout`, `/billing/portal`, `/admin/billing/stripe/connect` devuelven "no configurado" hasta agregar `STRIPE_API_KEY`). Verificado por testing_agent (100%).
  - **2026-09 — Historial de Benchmarks:** tabla `benchmark_runs` (migración `2026_09_benchmark_runs`); cada prueba se persiste (`_persist_benchmark`) y se lista en Salud del sistema vía `GET /admin/system/capacity/benchmark/history`. Verificado.
  - **2026-09 — Alertas de Capacidad:** watchdog en APScheduler cada 5 min (`services/capacity_alerts.py`, lock distribuido `capacity_alert_tick`). Si CPU o conexiones a la BD ≥ 80%, envía email vía Resend a **info@cortexiagt.com** (env `CAPACITY_ALERT_EMAIL`), con cooldown de 60 min por métrica (cache Redis). Umbral/intervalo/cooldown configurables por env. Verificado que la función mide y no alerta por debajo del umbral.
  - **2026-09 — Correo de bienvenida al crear clínica:** al crear una clínica, `create_clinic` genera un token de un solo uso (purpose='welcome', 72h) y envía por Resend un correo con el correo de acceso + enlace seguro para definir contraseña (SIN contraseña en texto plano) + recomendaciones. Best-effort: no bloquea la creación (`welcome_email_sent` en la respuesta). Plantilla `welcome_clinic_link`.
  - **2026-09 — Ver contraseña en login:** botón ojo (`toggle-password-visibility`) en `LoginPage.js`.
  - **2026-09 — Recuperación de contraseña:** flujo público con tokens de un solo uso (tabla `password_reset_tokens`, TTL 60 min, `services/password_tokens.py`). Endpoints: `POST /auth/forgot-password` (rate-limited; 404 con mensaje 'Este correo no está registrado. Hable con su administrador.' si no existe), `GET /auth/reset-token/validate`, `POST /auth/reset-password` (aplica política, actualiza vía Supabase admin, consume token). Páginas `ForgotPasswordPage.js` (/recuperar-password) y `ResetPasswordPage.js` (/restablecer-password?token=). El enlace de bienvenida reutiliza el mismo mecanismo. Verificado backend por curl (éxito, reuso bloqueado, login OK) + UI al 100%.
  - **2026-09 — Soporte in-app:** tablas `support_tickets` + `support_messages`. `routes/support.py`: usuarios con sesión crean tickets y responden (`/support/tickets`...); Super Admin lista todos, responde (envía notificación por correo `support_reply_notice`) y cambia estado (open/answered/closed) vía `/admin/support/...`. UI: `SupportPage.js` (clínica, /dashboard/soporte) y `AdminSupportPage.js` (super admin, /admin/soporte), con ítems de menú 'Soporte' en ambos layouts. Verificado end-to-end (curl + UI 100%).
  - **2026-09 — Soporte: adjuntos + badge:** los usuarios pueden adjuntar capturas de pantalla (imágenes, máx 5MB, hasta 5) al crear un ticket y en sus respuestas; se guardan en Supabase Storage (bucket `patient-files`, prefijo `_support/`) y se muestran como miniaturas en la conversación (usuario y super admin). Columna `support_messages.attachments` (JSONB). Endpoint `POST /support/upload`. Badge de tickets ABIERTOS en el menú 'Soporte' del Super Admin (`GET /admin/support/summary`, refresco cada 60s). NOTA: se agregó fallback de detección de tipo por firma de bytes en `services/input_sanitizer.py` porque libmagic no está instalado (mejora también la subida de logo). Verificado UI 100%.
  - **2026-09 — Fix 500 dashboard (maybe_single None):** el wrapper global de PostgREST (`core.py::_install_postgrest_retry`) ahora normaliza el `None` que devuelve `.maybe_single().execute()` con 0 filas a `_EmptyPostgrestResponse(data=None)`, eliminando los `AttributeError: NoneType .data` (500s) en el panel Super Admin (`require_clinic_member`, `get_clinic`, `list_users`, etc.). Verificado: super admin en endpoint de clínica → 403, clínica inexistente → 404, dashboard/usuarios → 200, sin nuevos 500.
  - **2026-09 — Notificación in-app de Soporte:** columna `support_tickets.user_unread`. Al responder el Super Admin se marca `user_unread=true`; al abrir el ticket el usuario lo marca `false`. `GET /support/unread-count`. El menú 'Soporte' de la clínica muestra un badge (`support-unread-badge`, refresco 60s) y en `SupportPage` el ticket con respuesta nueva se resalta con punto y etiqueta 'Nueva respuesta'. Verificado end-to-end (curl + UI 100%).
